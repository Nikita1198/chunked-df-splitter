...
from chunk_df import chunk_df_optimized
...
def test_basic_chunking(sample_df):
    ...
    chunks = list(chunk_df_optimized(sample_df, 1))
    assert len(chunks) == 3
...
def test_min_chunk_size(sample_df):
    ...
    chunks = list(chunk_df_optimized(sample_df, 3))
    assert len(chunks) == 2
...
def test_full_size_chunking(sample_df):
    ...
    chunks = list(chunk_df_optimized(sample_df, 6))
    assert len(chunks) == 1
    assert len(chunks[0]) == 6
    chunks = list(chunk_df_optimized(sample_df, 7))
    assert len(chunks) == 1
    assert len(chunks[0]) == 6
...
def test_empty_df(empty_df):
    ...
    chunks = list(chunk_df_optimized(empty_df, 1))
    assert len(chunks) == 0


def test_single_group(single_group_df):
    """Test chunking with a single datetime group."""
    chunks = list(chunk_df_optimized(single_group_df, 3))
    assert len(chunks) == 1
...
def test_unsorted_data(sample_df):
    ...
    chunks = list(chunk_df_optimized(unsorted_df, 3))
    assert len(chunks) == 2
...
def test_extra_columns(df_with_extra_cols):
    ...
    chunks_gen = list(chunk_df_optimized(df_with_extra_cols, 1))
...
def test_invalid_min_chunk_size(sample_df):
    ...
    with pytest.raises(ValueError, match="min_chunk_size must be positive"):
        list(chunk_df_optimized(sample_df, 0))
